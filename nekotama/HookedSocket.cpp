...
HookedSocketFactory& HookedSocketFactory::GetInstance()
{
	static std::unique_ptr<HookedSocketFactory> s_Factory;
	if (!s_Factory)
		s_Factory.reset(new HookedSocketFactory());
	return *(s_Factory.get());
}

HookedSocketFactory::HookedSocketFactory()
{
	WSADATA tData = { 0 };
...
SocketHandle HookedSocketFactory::Create(SocketType type)
{
	uint32_t tSocket;
	switch (type)
	{
	case SocketType::TCP:
		tSocket = Socket_Socket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
		break;
	case SocketType::UDP:
		tSocket = Socket_Socket(AF_INET, SOCK_DGRAM, IPPROTO_UDP);
		break;
	default:
	...
	return static_pointer_cast<ISocket>(shared_ptr<HookedSocket>(new HookedSocket(type, tSocket)));
}

bool HookedSocketFactory::Select(SocketHandleSet* read, SocketHandleSet* write, SocketHandleSet* error, uint32_t timeout)
{
...
HookedSocket::HookedSocket(SocketType type, uint32_t socket)
	: m_Type(type), m_Socket(socket)
{}

HookedSocket::~HookedSocket()
{
	Socket_CloseSocket(m_Socket);
}

void HookedSocket::IOControl(int32_t cmd, uint32_t* args)
{
	if (SOCKET_ERROR == ioctlsocket(m_Socket, cmd, (u_long*)args))
		throw logic_error(StringFormat("@%s: %s", "ioctlsocket", LASTWSAERR));
}

void HookedSocket::SetBlockingMode(bool blocking)
{
...
void HookedSocket::Bind(const char* addr, uint16_t port)
{
	sockaddr_in tAddr;
	tAddr.sin_family = AF_INET;
	tAddr.sin_port = htons(port);
	tAddr.sin_addr.S_un.S_addr = inet_addr(addr);
	if (SOCKET_ERROR == Socket_Bind(m_Socket, (sockaddr*)&tAddr, sizeof(tAddr)))
		throw logic_error(StringFormat("@%s: %s", "bind", LASTWSAERR));
}

void HookedSocket::Listen(uint32_t queue)
{
	if (SOCKET_ERROR == listen(m_Socket, queue))
		throw logic_error(StringFormat("@%s: %s", "listen", LASTWSAERR));
}

bool HookedSocket::Accept(std::string& ip, uint16_t& port, SocketHandle& handle)
{
	sockaddr_in tAddr;
	...
	{
		Socket_CloseSocket(tRet);
		return false;
	}
	ip = inet_ntoa(tAddr.sin_addr);
	port = ntohs(tAddr.sin_port);
	handle = static_pointer_cast<ISocket>(shared_ptr<HookedSocket>(new HookedSocket(SocketType::TCP, tRet)));
	return true;
}

void HookedSocket::Shutdown(SocketShutdownType how)
{
	int tRet;
	switch (how)
	{
	case SocketShutdownType::CloseRead:
		tRet = Socket_Shutdown(m_Socket, SD_RECEIVE);
		break;
	case SocketShutdownType::CloseWrite:
		tRet = Socket_Shutdown(m_Socket, SD_SEND);
		break;
	case SocketShutdownType::CloseReadWrite:
		tRet = Socket_Shutdown(m_Socket, SD_BOTH);
		break;
	default:
...
void HookedSocket::Connect(const char* addr, uint16_t port)
{
	sockaddr_in tAddr;
	tAddr.sin_family = AF_INET;
	tAddr.sin_port = htons(port);
...
bool HookedSocket::Recv(uint8_t* buffer, size_t length, size_t& readed, uint32_t flags)
{
...
bool HookedSocket::Send(const uint8_t* buffer, size_t length, size_t& wrote, uint32_t flags)
{
...
bool HookedSocket::TestIfClosed()
{
	char c;
